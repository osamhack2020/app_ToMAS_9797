...
public class SurveyResultContent extends Fragment {
    ...
    ArrayList<SurveyContentCustomView> customView_list;
                            ...
                            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                                if (task.isSuccessful()) {
                                    ...
                                    for (QueryDocumentSnapshot tmp_document : task.getResult()) {
                                        ...
                                        SurveyContentCustomView tmp_customView;
                                        if (tmp_type == 1)
                                        {
                                            tmp_customView = new SurveyContentCustomView(mainActivity, null, tmp_type, count, item_question, (ArrayList<String>) tmp_document.get("multi_choice_questions"));
                                        }
                                        else
                                        {
            ...
            public void onClick(View view) {
                ...
                mPostReference.collection("submissions").document(mainActivity.getUid()).set(post).addOnSuccessListener(new OnSuccessListener<Void>() {
                    ...
                    public void onSuccess(Void aVoid) {
                        ...
                        fragmentManager.beginTransaction().remove(SurveyResultContent.this).commit();
